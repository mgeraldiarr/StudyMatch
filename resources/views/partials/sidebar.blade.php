<aside class="sidebar" id="sidebar">
    ...
    @php
        ...
        $userMajor = $currentUser?->major ?? 'Belum diisi';
        $userUniv = $currentUser?->university ?? 'StudyMatch';
        $userAvatar = $currentUser?->avatar ?: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDJ_EKuVYe8DVz3xhNB1IDos9wyTPWBDFIz02_iHUF6jG9iAIhHpoTlStYIrxGaO7JJvxVdoqDvvgsnNDGdewZsm7P0aJpDbJdPhAeE70zUgqPekprvuVN3LRvsQZKdf_2qQ325NNi_EyAD4WxkKrLTYGHUzDhIQX9hpukdXn_hHh5YMCeZ2vJ303g49r7erhOHGRyCc3zArc8OI8kdOrtj7DpPqByRV-5mhVZZjJoC6A-w0B80CqPOq5bmN9RzH_CUf8Uhd-2DqEA';
    @endphp
    ...
    <div class="sb-user" data-url="{{ route('dashboard.user-profile') }}" onclick="window.location.href=this.dataset.url">
        ...
        <div>
            ...
            <div class="sb-usub">{{ $userMajor }}, {{ $userUniv }}</div>
        </div>
    ...
    <nav class="sb-nav">
        ...
        <form method="POST" action="{{ route('auth.logout') }}" style="display: contents;">
            @csrf
            <button type="submit" class="nav-item" style="background: none; border: none; width: 100%; text-align: left; cursor: pointer; font-family: inherit; font-size: inherit;">
